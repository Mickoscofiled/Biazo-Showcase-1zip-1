...
export default function PipesSteelCatalogue() {
  return (
    <section id="pipes-steel" className="py-16 bg-blue-50 dark:bg-slate-950 relative overflow-hidden">
      ...
      <div className="container mx-auto px-4 lg:px-8 max-w-7xl relative z-10">
        ...
        >
          ...
          <h2 className="text-2xl md:text-3xl font-bold text-slate-800 dark:text-slate-100 mb-2">Pipes, Steel & Chemicals</h2>
          ...
          <div className="flex flex-wrap justify-center gap-2 mt-5">
            {categories.map((cat) => (
              <a
                key={cat.id}
                href={`#cat-${cat.id}`}
                className="flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-blue-100 hover:bg-primary border border-blue-200 hover:border-primary text-slate-600 dark:text-slate-300 hover:text-white text-xs font-semibold transition-all"
              >
                {cat.label}
              </a>
            ))}
        ...
        <div className="grid md:grid-cols-3 gap-5">
          {categories.map((cat, i) => (
            <motion.div
              ...
              className="flex flex-col rounded-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/80 overflow-hidden shadow-sm hover:shadow-md transition-shadow pb-4"
            >
              {/* Card Header */}
              <div className="flex flex-col gap-3 px-5 py-4 border-b border-slate-100 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/50/50">
                <div className="flex items-center gap-3">
                  <div className="p-2 rounded-lg bg-blue-50 dark:bg-slate-950 border border-blue-100 dark:border-slate-800 shrink-0">
                    {cat.icon}
                  </div>
                  <div>
                    <h3 className="text-sm font-bold text-slate-800 dark:text-slate-100">{cat.title}</h3>
                    <p className="text-[10px] text-blue-500 font-semibold tracking-wide uppercase">{cat.subtitle}</p>
                  </div>
                </div>
              </div>

              <p className="px-5 pt-4 flex-1 text-xs text-slate-500 dark:text-slate-400 leading-relaxed">{cat.description}</p>
        ...
        >
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="shrink-0 flex items-center gap-2 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:bg-slate-800/50 text-primary px-5 py-2 rounded-full font-semibold text-xs transition-all hover:scale-105 shadow-md"
          >
