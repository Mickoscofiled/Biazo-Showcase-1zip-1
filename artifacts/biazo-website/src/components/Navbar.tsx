  ...
  return (
    <motion.header
      ...
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ? "bg-white/95 backdrop-blur-lg shadow-xl shadow-slate-200/50 border-b border-slate-100"
          : "bg-white/50 backdrop-blur-sm border-b border-slate-200/50"
      }`}
    >
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <nav className="hidden lg:flex items-center gap-4 xl:gap-6">
            ...
            <a
              ...
              className="flex items-center gap-2 bg-[#7391D1] hover:bg-[#607bb0] text-white px-5 py-2.5 rounded-full active:scale-95 active:translate-y-0 text-sm font-bold transition-all shadow-lg hover:shadow-xl hover:shadow-[#7391D1]/20 hover:-translate-y-0.5"
            >
          ...
          <div className="container mx-auto px-4 py-4 flex flex-col gap-1">
            {navLinks.map((link) => (
              <a
                ...
                className="text-sm font-medium text-slate-600 py-3 px-4 rounded-xl hover:bg-slate-50 hover:text-primary transition-colors"
              >
                {link.name}
              </a>
            ))}
